...
class Controller
{
    ...
    public function menu()
    {
        ...
        print '                        <img src="https://ui-avatars.com/api/?name=' . urlencode($_SESSION['usuario']) . '&background=ffffff&color=0d6efd&rounded=true&size=32" alt="avatar" class="rounded-circle me-2" />';
        print '                        <strong>' . $_SESSION['usuario'] . '</strong>';
        print '                    </a>';
    ...
    public function recuperarSenha($email)
    {
        ...
        $senha = random_int(1, 9)
            . str_pad(random_int(0, 9999999), 6, '0', STR_PAD_LEFT);
    ...
    // alterar usuario
    public function alterar_Usuario($id_usuario, $nome_usuario, $email, $senha, $id_perfil, $cpf, $telefone)
    {
        // Instancia a classe Usuario
        $objUsuario = new Usuario();
        // Validação do CPF
        if ($this->validarCPF($cpf) == false) {
            ...
            if ($objUsuario->alterarUsuario($id_usuario, $nome_usuario, $email, $senha, $id_perfil, $cpf, $telefone) == true) {
                session_start();
                // Carregar o menu
                $menu = $this->menu();
                // Incluir a view do usuário
    ...
    // funcao auxilixar para mascara cpf
    ...
    public function tabelaConsultaUsuario($resultado)
    {
        ...
        foreach ($resultado as $valor) {
            print '<tr>';
            print '<td>' . $valor->nome_usuario . '</td>';
            print '<td>' . $valor->email . '</td>';
            print '<td>' . $this->aplicarMascaraCPF($valor->cpf) . '</td>';
    ...
    public function modal_AlterarUsuario($id_usuario, $nome_usuario, $email, $id_perfil, $cpf, $telefone)
    {
        ...
        print '<input type="email" class="form-control" id="email_usuario_alterar" name="email_usuario"
        required autocomplete="off" placeholder="exemplo@dominio.com"
        ...
        print '<div class="col-md-6">';
        print '<label for="senha_usuario_alterar" class="form-label">Senha</label>';
        print '<input type="password" class="form-control" id="senha_usuario_alterar" name="senha"
        autocomplete="new-password" placeholder="Mínimo 6 caracteres (preencha para alterar)" pattern=".{6,}" title="A senha deve ter pelo menos 6 caracteres">';
        print '</div>';

        print '<div class="col-md-6">';
        print '<label for="confirma_senha_usuario_alterar" class="form-label">Confirme a Senha</label>';
        print '<input type="password" class="form-control" id="confirma_senha_usuario_alterar" name="confSenha"
        autocomplete="new-password" placeholder="Confirme a senha" pattern=".{6,}" title="A senha deve ter pelo menos 6 caracteres">';
        print '</div>';
    ...
    {
        print '<div class="modal fade" id="alterar_senha' . $id_usuario . '" tabindex="-1" aria-labelledby="alterarSenhaLabel" aria-hidden="true">';
        print '<div class="modal-dialog modal-dialog-centered">';
        print '<div class="modal-content rounded-3 shadow-sm">';

        // Cabeçalho
        print '<div class="modal-header bg-primary text-white">';
        print '<h5 class="modal-title" id="alterarSenhaLabel"><i class="bi bi-lock-fill me-2"></i>Alterar Senha</h5>';
        print '<button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>';
        print '</div>';
        ...
        print '<div class="alert alert-secondary text-center fw-bold">';
        print $nome_usuario;
        print '</div>';

        print '<div class="mb-3">';
        print '<label for="senha" class="form-label">Nova senha*</label>';
        print '<input type="password" class="form-control form-control-lg" name="senha" required>';
        print '</div>';

        print '<div class="mb-4">';
        print '<label for="confSenha" class="form-label">Confirmar nova senha*</label>';
        print '<input type="password" class="form-control form-control-lg" name="confSenha" required>';
        print '</div>';
    ...
    public function tabelaConsultarCliente($cliente)
    {
        ...
        foreach ($cliente as $valor) {
            print '<tr>';
            print '<td>' . $valor->nome_representante . '</td>';
            print '<td>' . $valor->nome_fantasia . '</td>';
            print '<td>' . $valor->email . '</td>';
            print '<td>' . $this->aplicarMascaraCNPJ($valor->cnpj_cliente) . '</td>';
            print '<td>' . $this->aplicarMascaraTelefone($valor->telefone_celular) . '</td>';
            print '<td>' . $this->aplicarMascaraTelefone($valor->telefone_fixo) . '</td>';
            // Limite de crédito com formatação
            $valor->limite_credito = number_format($valor->limite_credito, 2, ',', '.');
            // Exibe o limite de crédito
            print '<td>R$'  . $valor->limite_credito . '</td>';
            if ($this->temPermissao(['Administrador'])) {
                print '<td>';
                //botoes
                print '<div class="d-flex gap-2 justify-content-center flex-wrap">';
                print '<button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#alterar_cliente' . $valor->id_cliente . '">
                <i class="bi bi-pencil-square"></i><span class="d-none d-md-inline"></span>
                </button>';
    ...
    public function modalDetalhesCliente($cliente)
    {
        ...
        foreach ($cliente as $valor) {
            print '
            <div class="modal fade" id="detalhes_cliente' . $valor->id_cliente . '" tabindex="-1" aria-labelledby="detalhesClienteLabel' . $valor->id_cliente . '" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header bg-info text-white">
                            <h5 class="modal-title" id="detalhesClienteLabel' . $valor->id_cliente . '">Detalhes do Cliente</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                        </div>
                        <div class="modal-body">
                            <div class="container">
                                <div class="row mb-2">
                                    <div class="col-md-6"><strong>Nome do Representante:</strong> ' . $valor->nome_representante . '</div>
                                    <div class="col-md-6"><strong>Razão Social:</strong> ' . $valor->razao_social . '</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-md-6"><strong>Nome Fantasia:</strong> ' . $valor->nome_fantasia . '</div>
                                    <div class="col-md-6"><strong>CNPJ:</strong> ' . $this->aplicarMascaraCNPJ($valor->cnpj_cliente) . '</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-md-6"><strong>Email:</strong> ' . $valor->email . '</div>
                                    <div class="col-md-6"><strong>Inscrição Estadual:</strong> ' . $valor->inscricao_estadual . '</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-md-6"><strong>Limite de Crédito:</strong> R$ ' . $valor->limite_credito . '</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-md-6"><strong>Telefone Celular:</strong> ' . $this->aplicarMascaraTelefone($valor->telefone_celular) . '</div>
                                    <div class="col-md-6"><strong>Telefone Fixo:</strong> ' . $this->aplicarMascaraTelefone($valor->telefone_fixo) . '</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-md-4"><strong>Cidade:</strong> ' . $valor->cidade . '</div>
                                    <div class="col-md-4"><strong>Estado:</strong> ' . $valor->estado . '</div>
                                    <div class="col-md-4"><strong>Bairro:</strong> ' . $valor->bairro . '</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-md-6"><strong>CEP:</strong> ' . $this->aplicarMascaraCEP($valor->cep) . '</div>
                                    <div class="col-md-6"><strong>Complemento:</strong> ' . $valor->complemento . '</div>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
    ...
    public function modal_cadastroCliente()
    {
        ...
        print '<div class="col-md-6">';
        print '<label for="telefone_fixo" class="form-label">Telefone Fixo</label>';
        print '<input type="tel" class="form-control telefone_fixo" id="telefone_fixo" name="telefone_fixo" placeholder="(00) 0000-0000" pattern="\(\d{2}\) \d{4}-\d{4}" autocomplete="off">';
        print '</div>';
        ...
        print '</div>';
        print '<div class="col-md-6">';
        print '<button type="submit" class="btn btn-success w-100 py-2" id="cadastrar_cliente" name="cadastrar_cliente">';
        print '<i class="bi bi-check-circle"></i> Cadastrar';
        print '</button>';
        print '</div>';
        print '</div>';
    ...
    public function aprovar_Pedido($id_pedido, $origem)
    {
        ...
        if ($objPedido->aprovarPedido($id_pedido) == true) {
            session_start();
            // Incluir a view
            if ($origem === 'pedido') {
                // Carregar o menu
            ...
            // Exibir mensagem de sucesso
            $this->mostrarMensagemSucesso("Pedido aprovado com sucesso");
        } else {
            session_start();
            // Carregar o menu
            $menu = $this->menu();
            ...
            // Exibir mensagem de erro
            $this->mostrarMensagemErro("Erro ao aprovar Pedido");
        }
    }
    ...
    // Método para buscar produtos abaixo do estoque mínimo
